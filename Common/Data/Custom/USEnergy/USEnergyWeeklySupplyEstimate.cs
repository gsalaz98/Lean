...
namespace QuantConnect.Data.Custom.USEnergy
{
    ...
    public class USEnergyWeeklySupplyEstimate : BaseData
    {
        ...
        public override BaseData Reader(SubscriptionDataConfig config, string line, DateTime date, bool isLiveMode)
        {
            ...
            if (!DateTime.TryParseExact(csv[0], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out time))
            {
                return null;
            }
            if (!decimal.TryParse(csv[1], NumberStyles.Float, CultureInfo.InvariantCulture, out value))
            {
                return null;
            }
