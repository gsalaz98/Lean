...
namespace QuantConnect.ToolBox.BrainDataConverter
{
    public class BrainDataProgram
    {
        ...
        public static void BrainDataConverter(string date, string sourceDirectory, string destinationDirectory)
        {
            ...
            var converter = new BrainDataConverter(sourceDirectoryInfo, destinationDirectoryInfo);
            if (!converter.Convert(dateOf))
            {
                Log.Error("BrainDataProgram.BrainDataProgram(): Failed to convert BrainData data");
            }
        }
    }
}
