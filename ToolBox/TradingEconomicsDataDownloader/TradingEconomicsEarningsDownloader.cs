        ...
        public override bool Run()
        {
            ...
            foreach (var kvp in data.GroupBy(GetFileName))
            {
                ...
                try
                {
                    ...
                    Log.Trace($"TradingEconomicsEarningsDownloader.Run(): Writing file before compression: {path}");
                    File.WriteAllText(path, contents);

                    Log.Trace($"TradingEconomicsEarningsDownloader.Run(): Compressing to: {zipPath}");
                    // Write out this data string to a zip file
                    Compression.Zip(path, zipPath, kvp.Key, true);
                }
                catch (Exception e)
                {
                    Log.Error(e, $"TradingEconomicsEarningsDownloader.Run(): Error creating {path}");
                    return false;
                }
            }
