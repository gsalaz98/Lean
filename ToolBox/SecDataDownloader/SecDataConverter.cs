...
namespace QuantConnect.ToolBox.SecDataDownloader
{
    public class SecDataConverter
    {
        ...
        public SecDataConverter(string knownEquityFolder)
        {
        ...
        /// </summary>
        /// <param name="rawSource">Source of raw files</param>
        /// <param name="destination">Destination of our json files</param>
        /// <param name="postProcess">Determines if we should try to get the publication time of the report</param>
        public void Process(string rawSource, string destination, bool postProcess)
        {
            Parallel.ForEach(
                ...
                rawFile =>
                {
                    ...
                    Parallel.ForEach(
                        Directory.GetFiles(extractDataPath, "*.nc", SearchOption.AllDirectories).ToList(),
                        rawReportFilePath =>
                        {
                            ...
                            {
                                Log.Error($"SecDataConverter.Process(): Failed to parse XML from file path: {rawReportFilePath}");
                                Log.Error(e);
                                return;
                            }
                    ...
                    Parallel.ForEach(
                        ...
                        ticker =>
                        {
                            ...
                            WriteReport(reports, ticker, destination);
                        }
                    );
        ...
        /// <param name="useCache">Should use existing cache. If the company CIK is not found, we will look up the company anyways</param>
        ...
        public void PostProcess(ISecReport report)
        {
            var companyCik = report.Report.Filers.First().CompanyData.Cik;
            ...
            if (companyPublicationDates.TryGetValue(
                report.Report.AccessionNumber.Replace("-", ""),
                out reportPublicationDate
            ))
            {
        ...
        public void WriteReport(List<ISecReport> reports, string ticker, string destination)
        {
        ...
        private Dictionary<string, DateTime> GetReportPublicationTimes(string cik)
        {
            for (var retries = 0; retries < MaxRetries; retries++)
            {
                try
                {
                    using (var client = new WebClient())
                    {
                        var data = client.DownloadString($"{BaseUrl}/{cik}/index.json");
                        var metadata = JObject.Parse(data);
                        var items = metadata["directory"].ToObject<SecReportMetadata>();
                ...
                catch (WebException e)
                {
                    ...
                    {
                        Log.Error($"SecDataConverter.GetReportPublicationTimes(): Submissions not found for CIK: {cik}, trying again...");
                    }
                }
            }

            throw new Exception($"SecDataConverter.GetReportPublicationTimes(): Submission not found for CIK: {cik}");
        }
    }
}
